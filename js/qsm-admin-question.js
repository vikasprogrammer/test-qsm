...
(function ($) {
	...
	QSMQuestion = {
			...
			$.ajax( {
				...
				data: {
					'quiz_id' :0,
                                        ...
                                        'category' : $('#question-bank-cat-2').val()
				},
		...
		loadQuestionBank1: function( action = '' ) {
			if( action == 'change' ){
				$( '.qb-load-more-wrapper' ).remove();
				$( '#question-bank' ).find( '.question-bank-question' ).remove();
				$( '#question-bank' ).append( '<div style="top: 45px;position: relative;" class="qsm-spinner-loader"></div>' );
...
$.ajax( {
	url: wpApiSettings.root + 'quiz-survey-master/v1/quizz_questions/0/',
	method: 'GET',
	beforeSend: function ( xhr ) {
		xhr.setRequestHeader( 'X-WP-Nonce', qsmQuestionSettings.nonce );
	},
	data: {
		'quizID' :0 ,
							'page' : $('#question_back_page_number').length > 0 ? parseInt( $('#question_back_page_number').val() ) + 1 : 1,
							'category' :$('#question-bank-cat-1').val()
	},
	success: QSMQuestion.questionBankLoadSuccess
			...
			for ( var i = 0; i < questions.length; i++) {
				QSMQuestion.addQuestionToQuestionBank( questions[i] );                                
                        ...
                        if(pagination.current_page == 1 && qsmQuestionSettings.categories.length > 0){
							...
							var quizNaems = qsmQuestionSettings.quizNaems;  
						                  
							$cat_html = '<select name="question-bank-cat" id="question-bank-cat-2">';
							if(val_quiz1 == 0){   
							$cat_html += '<option value="" selected="selected">All Questions</option>';
							} else {
								$cat_html += '<option value="">All Questions</option>';
							}
                            $.each(category_arr, function(index, value){
								if(value.category !== '')
								if(value.category == val_quiz1){
									$cat_html += '<option value="'+ value.category +'" selected="selected">'+ value.category +' Questions</option>';
								}
								else {
							...
							$cat_html += '</select>';
							//this is for quiz data
							$cat_html += '<select name="question-bank-cat-1" id="question-bank-cat-1">';
							if(val_quiz == 0){   
								$cat_html += '<option value="" selected="selected">All Quizzes</option>';
								} else {
									$cat_html += '<option value="">All Quizzes</option>';
								}
                            $.each(quizNaems, function(index, value){
								if(value.quiz_id !== '')
								if(value.quiz_id == val_quiz){
									$cat_html += '<option value="'+ value.quiz_id +'" selected="selected" >'+ value.quiz_name +' Quiz</option>';
								} else {
									$cat_html += '<option value="'+ value.quiz_id +'"  >'+ value.quiz_name +' Quiz</option>';
								}
                                    
                            });
							$cat_html += '</select>';
							console.log(qsmQuestionSettings);
							for (var i in qsmQuestionSettings) 
								{
									console.log("row " + i);
								}
		...
		savePages: function() {
			...
			var data = {
				...
				quiz_name:qsmQuestionSettings.quizName,
	...
	$(function() {
		...
		$( '.questions' ).on( 'click', '.add-question-bank-button', function( event ) {
			...
			QSMQuestion.loadQuestionBank();
                ...
                $( document ).on( 'change', '#question-bank-cat-2', function( event ) {
			event.preventDefault();
		val_quiz1 = 	$('#question-bank-cat-2').val() ;
		val_quiz =0
			QSMQuestion.loadQuestionBank('change');
		});
		$( document ).on( 'change', '#question-bank-cat-1', function( event ) {
			event.preventDefault();

		val_quiz = 	$('#question-bank-cat-1').val() ;
			QSMQuestion.loadQuestionBank1('change');
			val_quiz1 = 0;
	});

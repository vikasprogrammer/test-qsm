...
function qsm_register_rest_routes() {
	...
	register_rest_route( 'quiz-survey-master/v1', '/quizz_questions/(?P<id>\d+)', array(
		'methods'  => WP_REST_Server::READABLE,
		'callback' => 'qsm_rest_get_bank_questions1',
                'permission_callback' => function () {
                    return current_user_can( 'edit_posts' );
                }
	) );
 ...
 */
function qsm_rest_get_bank_questions1( WP_REST_Request $request ){
    if(is_user_logged_in()){
        global $wpdb;
        $category = isset($_REQUEST['category']) ? $_REQUEST['category'] : '';
        $category_query = '';
        if($category){
            $category_query = ' AND quiz_id = "' . $category . '"';
        }
